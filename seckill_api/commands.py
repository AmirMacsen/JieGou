...
import asyncio
from utils.cache import JieGouRedis

jiegou_redis = JieGouRedis()
...
async def init_seckill_ing_redis():
    ...
    async with AsyncSessionFactory() as session:
        async with session.begin():
            ...
            session.add(seckill)
    # 2. 把秒杀数据也要同步到redis
    await jiegou_redis.add_seckill(seckill)
    await jiegou_redis.init_stock(seckill.id, seckill.stock)
